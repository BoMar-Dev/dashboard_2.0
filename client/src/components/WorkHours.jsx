      ...
      {hours.map((hour, index) => (
        <div key={index} className="justify-start items-center gap-[5.63px] inline-flex">
          ...
          <div className="w-[140.15px] h-[0px] border border-thin border-neutral-500 ">
            ...
            {currentDay &&
              meetings
                .filter((meeting) => meeting.day && meeting.day.toLowerCase() === currentDay.toLowerCase())
                .map((meeting) => {
                  const meetingFromTime = parseInt(meeting.fromTime.split(":")[0]);
                  const meetingToTime = parseInt(meeting.toTime.split(":")[0]);
                  if (hour >= meetingFromTime && hour < meetingToTime) {
                    return (
                      <div key={meeting.id} className={`${styles.meetingStyle}`}>
                        {meeting.title}
                        <br />
                        {meeting.place}
                      </div>
